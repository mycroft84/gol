...
abstract class Kohana_ORM_Nested_Sets extends ORM{
    ...
    {
        return $this->_and_scope(
            self::factory($this->object_name())
                ->where($this->_right_column, '=', $this->{$this->_left_column} - 1)
        )->find();
    }
    ...
    public function get_next_sibling()
    {
        return $this->_and_scope(
            self::factory($this->object_name())
                ->where($this->_left_column, '=', $this->{$this->_right_column} + 1)
        )->find();
    }
    ...
    public function get_leafs()
    {
        return $this->_and_scope(
            self::factory($this->object_name())
                ->where(DB::expr($this->_right_column." - ".$this->_left_column), '=', 1)
        )->find_all();
    }
    ...
    public function get_first_child()
    {
        return $this->_and_scope(
            self::factory(self::object_name())
                ->where($this->_left_column, '=', $this->{$this->_left_column} + 1)
        )->find();
    }
    ...
    public function get_last_child()
    {
        return $this->_and_scope(
            self::factory(self::object_name())
                ->where($this->_right_column, '=', $this->{$this->_right_column} - 1)
        )->find();
    }
    ...
    {
        /** @var ORM $result **/
        $result = $this->_and_scope(
            self::factory(self::object_name())
                ->where($this->_left_column, $include_node ? '>=' : '>', $this->{$this->_left_column})
                ->and_where($this->_right_column, $include_node ? '<=' : '<', $this->{$this->_right_column})
        )->order_by($this->_left_column, 'ASC');

        if ($depth !== null)
        {
            $result->and_where($this->_level_column, '<=', $this->{$this->_level_column} + $depth);
        }
    ...
    public function get_parent()
    {
        return $this->_and_scope(
            self::factory(self::object_name())
                ...
                ->and_where($this->_level_column, '>=', $this->{$this->_level_column} - 1)
        )->order_by($this->_right_column, 'asc')->find();
    }
    ...
    {
        return ($this->{$this->_right_column} - $this->{$this->_left_column} - 1) / 2;
    }
    ...
    public function insert_as_parent_of($node)
    {
        ...
        $new_left = $node->{$this->_left_column};
        $new_right = $node->{$this->_right_column} + 2;
        $new_scope = $node->{$this->_scope_column};
        $new_level = $node->{$this->_level_column};
        ...
        try
        {
            ...
            $this->_shift_rl_values($node->{$this->_right_column} + 1, 2, $new_scope);

            // slide child nodes over one and down one to allow new parent to wrap them
            $this->_and_scope(
                DB::update($this->table_name())
                    ->value($this->_left_column, DB::expr("{$this->_left_column} + 1"))
                    ->value($this->_right_column, DB::expr("{$this->_right_column} + 1"))
                    ->value($this->_level_column, DB::expr("{$this->_level_column} + 1"))
                    ->and_where($this->_left_column, '>=', $new_left)
                    ->and_where($this->_right_column, '<=', $new_right),
                $new_scope
            )->execute($this->_db);

            $this->{$this->_level_column} = $new_level;
            $this->_insert_node($new_left, $new_right, $new_scope);
    ...
    public function insert_as_prev_sibling_of($node)
    {
        ...
        $new_left = $node->{$this->_left_column};
        $new_right = $node->{$this->_left_column} + 1;
        $new_scope = $node->{$this->_scope_column};
        ...
        try
        {
            ...
            $this->_shift_rl_values($new_left, 2, $new_scope);
            $this->{$this->_level_column} = $node->{$this->_level_column};
            $this->_insert_node($new_left, $new_right, $new_scope);

            $this->_db->commit();

            // upgrade node right, left values
            $node->{$this->_left_column} += 2;
            $node->{$this->_right_column} += 2;
        }
        catch (Exception $e)
        {
            $this->_db->rollback();
    ...
    public function insert_as_next_sibling_of($node)
    {
        ...
        $new_left = $node->{$this->_right_column} + 1;
        $new_right = $node->{$this->_right_column} + 2;
        $new_scope = $node->{$this->_scope_column};
    ...
    public function insert_as_first_child_of($node)
    {
        ...
        $new_left = $node->{$this->_left_column} + 1;
        $new_right = $node->{$this->_left_column} + 2;
        $new_scope = $node->{$this->_scope_column};
        ...
        try
        {
            ...
            $this->_shift_rl_values($new_left, 2, $new_scope);
            $this->{$this->_level_column} = $node->{$this->_level_column} + 1;
            $this->_insert_node($new_left, $new_right, $new_scope);
    ...
    public function insert_as_last_child_of($node)
    {
        ...
        $new_left = $node->{$this->_right_column};
        $new_right = $node->{$this->_right_column} + 1;
        $new_scope = $node->{$this->_scope_column};
     ...
     * @return bool
     */
    protected function _move_between_trees($node, $new_left_value, $move_type)
    {
        try
        {
            $this->_db->begin();
            ...
            $new_scope = $node->{$this->_scope_column};
            $old_scope = $this->{$this->_scope_column};
            $old_lft = $this->{$this->_left_column};
            $old_rgt = $this->{$this->_right_column};
            $old_level = $this->{$this->_level_column};

            // prepare target tree for insertion, make room
            $this->_shift_rl_values($new_left_value, $old_rgt - $old_lft - 1, $new_scope);

            // set new root id for this node
            $this->{$this->_scope_column} = $new_scope;
            parent::save();

            // insert this node as a new node
            $this->{$this->_right_column} = 0;
            $this->{$this->_left_column} = 0;
            ...
            $this->{$this->_right_column} = $this->{$this->_left_column} + $old_rgt - $old_lft;
            parent::save();

            $new_level = $this->{$this->_level_column};
            $level_diff = $new_level - $old_level;

            // relocate descendants of the node
            $diff = $this->{$this->_left_column} - $old_lft;

            // update lft/rgt/root/level for all descendants
            $update = DB::update($this->table_name())
                ->value($this->_left_column, DB::expr("{$this->_left_column} + $diff"))
                ->value($this->_right_column, DB::expr("{$this->_right_column} + $diff"))
                ->value($this->_level_column, DB::expr("{$this->_level_column} + $level_diff"));
            if ($this->_use_scope)
            {
                $update->value($this->_scope_column, $new_scope);
            }
            $this->_and_scope(
                $update->where($this->_left_column, '>', $old_lft)
                    ->and_where($this->_right_column, '<', $old_rgt),
                $old_scope
            )->execute($this->_db);
            ...
            $this->_shift_rl_values($first, $delta, $old_scope);
    ...
    public function move_as_prev_sibling_of($node)
    {
        ...
        if ($node->{$this->_scope_column} != $this->{$this->_scope_column})
        {
            // move between trees
            return $this->_move_between_trees($node, $node->{$this->_left_column}, __FUNCTION__);
        }
        else
        {
            ...
            $old_level = $this->{$this->_level_column};
            $this->{$this->_level_column} = $node->{$this->_level_column};
            $this->_update_node($node->{$this->_left_column}, $this->{$this->_level_column} - $old_level);
        }
    ...
    public function move_as_next_sibling_of($node)
    {
        ...
        {
            // move between trees
            return $this->_move_between_trees($node, $node->{$this->_right_column} + 1, __FUNCTION__);
        }
        else
        {
            // move within tree
            $old_level = $this->{$this->_level_column};
            $this->{$this->_level_column} = $node->{$this->_level_column};
            $this->_update_node($node->{$this->_right_column} + 1, $this->{$this->_level_column} - $old_level);
        }
    ...
    public function move_as_first_child_of($node)
    {
        ...
        {
            // move between trees
            return $this->_move_between_trees($node, $node->{$this->_left_column} + 1, __FUNCTION__);
        }
        else
        {
            // move within tree
            $old_level = $this->{$this->_level_column};
            $this->{$this->_level_column} = $node->{$this->_level_column} + 1;
            $this->_update_node($node->{$this->_left_column} + 1, $this->{$this->_level_column} - $old_level);
        }
    ...
    public function move_as_last_child_of($node)
    {
        ...
        {
            // move between trees
            return $this->_move_between_trees($node, $node->{$this->_right_column}, __FUNCTION__);
        }
        else
        {
            // move within tree
            $old_level = $this->{$this->_level_column};
            $this->{$this->_level_column} = $node->{$this->_level_column} + 1;
            $this->_update_node($node->{$this->_right_column}, $this->{$this->_level_column} - $old_level);
        }
    ...
    public function make_root($new_scope = NULL)
    {
        ...
        // check scope
        if ($this->_use_scope AND empty($new_scope))
        {
            $new_scope = $this->_get_next_scope();
        }

        $old_rgt = intval($this->{$this->_right_column});
        $old_lft = intval($this->{$this->_left_column});
        $old_level = intval($this->{$this->_level_column});
        $old_scope = intval($this->{$this->_scope_column});
        ...
        try
        {
            ...
            if ($this->loaded())
            {
                $update = DB::update($this->table_name())
                    ->value($this->_left_column, DB::expr("{$this->_left_column} + $diff"))
                    ->value($this->_right_column, DB::expr("{$this->_right_column} + $diff"))
                    ->value($this->_level_column, DB::expr("{$this->_level_column} + $old_level"));
                if ($this->_use_scope)
                {
                    $update->value($this->_scope_column, $new_scope)->where($this->_scope_column, '=', $old_scope);
                }
                $update->and_where($this->_left_column, '>', $old_lft)
                    ->and_where($this->_right_column, '<', $old_rgt)
                    ->execute($this->_db);
            ...
            $this->{$this->_right_column} = $this->loaded() ? $old_rgt - $old_lft + 1 : 2;
            $this->{$this->_scope_column} = $new_scope;
    ...
    public function is_equal_to($node)
    {
        ...
        return (($this->{$this->_left_column} == $node->{$this->_left_column}) AND
                ($this->{$this->_right_column} == $node->{$this->_right_column}) AND
                (!$this->_use_scope OR ($this->{$this->_scope_column} == $node->{$this->_scope_column})));
    }
    ...
    public function is_ancestor_of($node)
    {
        ...
        return (($node->{$this->_left_column} > $this->{$this->_left_column}) AND
                ($node->{$this->_right_column} < $this->{$this->_right_column}) AND
                (!$this->_use_scope OR ($node->{$this->_scope_column} == $this->{$this->_scope_column})));
    }
    ...
    {
        return intval($this->{$this->_right_column}) > intval($this->{$this->_left_column});
    }
     ...
     * @return Kohana_ORM_Nested_Sets
     */
    protected function _get_node($node)
    {
        $old_node = $node;
     ...
     * @return $this
     */
    protected function _insert_node($left = 0, $right = 0, $scope = 1)
    {
        $this->{$this->_left_column} = $left;
        $this->{$this->_right_column} = $right;
     ...
     * @return bool
     */
    protected function _update_node($node_left, $level_diff)
    {
        $left = $this->{$this->_left_column};
        $right = $this->{$this->_right_column};
        ...
        try
        {
            ...
            $this->_shift_rl_values($node_left, $tree_size, $scope);
            ...
            $this->_and_scope(
                DB::update($this->table_name())
                    ->value($this->_level_column, DB::expr("{$this->_level_column} + $level_diff"))
                    ->where($this->_left_column, '>', $left)
                    ->and_where($this->_right_column, '<', $right)
            )->execute($this->_db);

            // now there's enough room next to target to move the subtree
            $this->_shift_rl_range($left, $right, $node_left - $left, $scope);

            // correct values after source (close gap in old tree)
            $this->_shift_rl_values($right + 1, -$tree_size, $scope);
     ...
     * @return $this
     */
    protected function _shift_rl_values($first, $delta, $scope)
    {
        $this->_and_scope(
            DB::update($this->table_name())
                ->value($this->_left_column, DB::expr("{$this->_left_column} + $delta"))
                ->where($this->_left_column, '>=', $first),
            $scope
        )->execute($this->_db);

        $this->_and_scope(
            DB::update($this->table_name())
                ->value($this->_right_column, DB::expr("{$this->_right_column} + $delta"))
                ->where($this->_right_column, '>=', $first),
            $scope
        )->execute($this->_db);
     ...
     * @return $this
     */
    protected function _shift_rl_range($first, $last, $delta, $scope)
    {
        $this->_and_scope(
            DB::update($this->table_name())
                ->value($this->_left_column, DB::expr("{$this->_left_column} + $delta"))
                ->where($this->_left_column, '>=', $first)
                ->and_where($this->_left_column, '<=', $last),
            $scope
        )->execute($this->_db);
        ...
        $this->_and_scope(
            DB::update($this->table_name())
                ->value($this->_right_column, DB::expr("{$this->_right_column} + $delta"))
                ->where($this->_right_column, '>=', $first)
                ->and_where($this->_right_column, '<=', $last),
            $scope
        )->execute($this->_db);
     ...
     * @return mixed
     */
    protected function _and_scope($object, $scope = NULL)
    {
        if ($this->_use_scope)
        {
            $object->and_where($this->_scope_column, '=', is_null($scope) ? $this->{$this->_scope_column} : $scope);
        }
     ...
     * @return int
     */
    protected function _get_next_scope()
    {
        // returns available value for scope
        $scope = DB::select(DB::expr("MAX({$this->_scope_column}) as scope"))
    ...
    {
        if ($column == $this->_scope_column AND !$this->_use_scope)
        {
            return NULL;
        }
